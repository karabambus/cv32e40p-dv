   ...
   if (mon_trn.access_type == UVMA_OBI_MEMORY_ACCESS_WRITE) begin
      ...
      case (get_vp_index(mon_trn))
         0: begin
            ...
            cv32e40p_cntxt.vp_status_vif.exit_value = mon_trn.data[0];
            cv32e40p_cntxt.vp_status_vif.exit_valid = 1;
            if (mon_trn.data[0] == 0) begin
               `uvm_info("VP_IDX_0", $sformatf("VP Status Flags: TEST PASSED WITH CODE %h", cv32e40p_cntxt.vp_status_vif.exit_value), UVM_NONE)
            end
            else begin
               `uvm_error("VP_IDX_0", $sformatf("VP Status Flags: TEST FAILED WITH CODE %h", cv32e40p_cntxt.vp_status_vif.exit_value))
            end
         end
         1: begin
            ...
            cv32e40p_cntxt.vp_status_vif.exit_value = mon_trn.data[0];
            cv32e40p_cntxt.vp_status_vif.exit_valid = 1;
            `uvm_info("VP_IDX_1", $sformatf("VP Status Flags: TEST PASSED WITH CODE %h", cv32e40p_cntxt.vp_status_vif.exit_value), UVM_NONE)
         end
         default: begin
