   ...
   if (cfg.is_active) begin
      fork
         ...
         begin : obi_data_slv_thread
            ...
            //   if (!$cast(vp_seq, data_slv_seq.register_vp_vseq("vp_rand_num", cfg.vp_rand_num, uvme_cv32e40p_vp_rand_num_seq_c::get_type()))) begin
            ...
            begin
               ...
               if (!$cast(vp_seq, data_slv_seq.register_vp_vseq("vp_sig_writer", cfg.vp_sig_writer, uvme_cv32e40p_vp_sig_writer_seq_c::get_type()))) begin
            ...
            begin
                if (vp_status_flags_symbol_present) begin
                    ...
                    `uvm_info("CV32E40PVPSEQ", $sformatf("Setting up vp_status with addr %h", cfg.vp_status_flags_symbol), UVM_LOW)
                    if (!$cast(vp_seq, data_slv_seq.register_vp_vseq("vp_status_flags", cfg.vp_status_flags_symbol, uvme_cv32e40p_vp_status_flags_seq_c::get_type()))) begin
            ...
            begin
               uvme_cv32e40p_vp_status_flags_seq_c vp_seq;
               `uvm_info("CV32E40PVPSEQ", $sformatf("Setting up vp_status with addr %h", cfg.vp_status_flags_legacy), UVM_LOW)
               if (!$cast(vp_seq, data_slv_seq.register_vp_vseq("vp_status_flags", cfg.vp_status_flags_legacy, uvme_cv32e40p_vp_status_flags_seq_c::get_type()))) begin
                  `uvm_fatal("CV32E40PVPSEQ", $sformatf("Could not cast vp_status_flags correctly"));
               end
               vp_seq.cv32e40p_cntxt = cntxt;
            end
            ...
            begin
               ...
               if (!$cast(vp_seq, data_slv_seq.register_vp_vseq("vp_interrupt_timer", cfg.vp_interrupt_timer, uvme_cv32e40p_vp_interrupt_timer_seq_c::get_type()))) begin
            ...
            begin
               ...
               if (!$cast(vp_seq, data_slv_seq.register_vp_vseq("vp_debug_control", cfg.vp_debug_control, uvme_cv32e40p_vp_debug_control_seq_c::get_type()))) begin
...
function void uvme_cv32e40p_env_c::load_binary();
    ...
    if ($value$plusargs("elf_file=%s", binary)) begin
        `uvm_info("cv32e40p_env", $sformatf("\n\t\t\tReading ELF: %s", binary) , UVM_NONE)
        read_elf(binary);
        vp_status_flags_symbol_present = ! read_symbol("tohost", cfg.vp_status_flags_symbol);
        if (vp_status_flags_symbol_present) begin
            `uvm_info("cv32e40p_env", $sformatf("Loading tohost symbol: %h", cfg.vp_status_flags_symbol), UVM_LOW)
        end else begin
            `uvm_info("cv32e40p_env", "tohost symbol not present" , UVM_LOW)
        end
        ...
        if (vp_virtual_printer_symbol_present) begin
            `uvm_info("cv32e40p_env", $sformatf("Loading stdout_reg symbol: %h", cfg.vp_virtual_printer_symbol), UVM_LOW)
        end else begin
            `uvm_info("cv32e40p_env", "stdout_reg symbol not present" , UVM_LOW)
        end
    end
